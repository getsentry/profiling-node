...
const DEFAULT_HUB_GETTER = () => undefined;
class ProfilingIntegration {
    constructor() {
        this.name = INTEGRATION_NAME;
        this.getCurrentHub = DEFAULT_HUB_GETTER;
    }
    ...
    handleGlobalEvent(event) {
        if ((0, utils_2.isProfiledTransactionEvent)(event)) {
            ...
            if (!hub) {
                utils_1.logger.log('[Profiling] getCurrentHub did not return a Hub, removing profile from event and forwarding to next event processors.');
