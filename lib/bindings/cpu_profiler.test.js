...
const profiled = (name, fn) => {
    cpu_profiler_1.CpuProfilerBindings.startProfiling(name);
    fn();
    return cpu_profiler_1.CpuProfilerBindings.stopProfiling(name);
};
...
describe('Profiler bindings', () => {
    it('exports profiler binding methods', () => {
        expect(typeof cpu_profiler_1.CpuProfilerBindings['startProfiling']).toBe('function');
        expect(typeof cpu_profiler_1.CpuProfilerBindings['stopProfiling']).toBe('function');
        expect(typeof cpu_profiler_1.CpuProfilerBindings['setSamplingInterval']).toBe('function');
        expect(typeof cpu_profiler_1.CpuProfilerBindings['setUsePreciseSampling']).toBe('function');
    });
